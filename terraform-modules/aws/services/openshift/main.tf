...
resource "aws_instance" "os" {
  ...
  availability_zone           = var.multiple_az && var.instance_count <= 6 ? "${var.region}${var.az_map[count.index + 1]}" : "${var.region}${var.availability_zone}"
...
resource "aws_ebs_volume" "dp" {
  ...
  lifecycle {
    ignore_changes = [
      # Ignorar mudanças no atributo "size"
      size,
    ]
  }
}
